...
function InterviewAgent({
  ...
  useEffect(() => {
    ...
    const onCallEnd = () => {
      console.log("Call ended. Total messages:", messages.length);
    ...
    const onMessage = (message: Message) => {
      if (message.type === "transcript" && message.transcriptType === "final") {
        ...
        console.log(`Message ${messages.length + 1}:`, message.role, message.transcript);
    ...
    const onError = (err: Error) => {
      ...
      // Check if it's a meeting ejection error
      if (err.message?.includes("Meeting ended") || err.message?.includes("ejection")) {
        toast.error("Interview session ended. This may be due to time limits or inactivity. Your progress has been saved.");
      } else {
    ...
    <>
      <div className="call-view">
        ...
        <section className="card-interviewer">
          ...
          <h3 className="capitalize">Hire-Wise</h3>
        </section>
